...
export default function WhatsAppSettingsPage() {
  ...
  const copyToClipboard = async (value: string) => {
    try {
      ...
      setToast({ type: 'error', text: 'Failed to copy to clipboard.' });
    }
  };
  ...
  const handleSave = () => {
    ...
    try {
      ...
      setToast({ type: 'success', text: 'WhatsApp settings saved successfully!' });
    } catch (err) {
      setSaveStatus("idle");
      setToast({ type: 'error', text: 'Failed to save settings. Your browser storage may be full or restricted.' });
    }
  };
  ...
  const handleRegenerateCredentials = () => {
    ...
    setShowToken(false);
    setToast({ type: 'success', text: 'New access token generated. Remember to save your settings.' });
  };

  const handleUnlinkDevice = () => {
    setIsLinked(false);
    setShowToken(false);
    setToast({ type: 'success', text: 'Device unlinked.' });
  };

  const handleResetInvoiceTemplate = () => {
    setSelectedTemplate("standard");
    setInvoiceTemplate(INVOICE_TEMPLATES.standard);
    setToast({ type: 'success', text: 'Invoice template reset to default.' });
  };
              ...
              <div className="bg-surface border border-outline-variant/30 rounded-[2rem] p-1">
                <div className="relative h-full bg-surface-container-lowest rounded-[1.8rem] p-6 sm:p-8">
                  <div className="flex items-center gap-3 mb-8 pb-4 border-b border-outline-variant/20">
                    <div className="flex items-center justify-center border border-tertiary/20 bg-tertiary/10 rounded-2xl w-12 h-12 text-tertiary">

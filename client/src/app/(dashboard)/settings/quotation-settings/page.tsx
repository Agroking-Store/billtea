  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          let msg = data?.message || "Failed to load settings";
        ...
        if (data && data.settings) {
          ...
          setStartingNumber(data.settings.nextNumber?.toString() || "1001");
        ...
        setToast({ type: 'error', text: error.message || 'Failed to load settings' });
      } finally {
        setLoading(false);
      }
    };
    ...
    try {
      const res = await apiFetch(`/document-settings/${selectedBranchId}`, {
        ...
        body: JSON.stringify({
          type: "QUOTATION",
          prefix,
          nextNumber: parseInt(startingNumber) || 1001,
          topMessage,
          bottomMessage,
          terms,
          showSku: sku,
      ...
      if (!res.ok) {
        let msg = data?.message || `Failed to save settings (status ${res.status})`;
        if (Array.isArray(msg)) msg = msg.join(", ");
        if (res.status === 401) msg = "Your session has expired. Please log in again.";
        if (res.status === 403) msg = "You do not have permission to update these settings.";
        throw new Error(msg);
      }

      setToast({ type: 'success', text: 'Quotation settings saved successfully!' });
    } catch (error: any) {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-2">
                        ...
                        <p className="text-[12px] text-on-surface-variant/70 mt-1 font-medium">
                          Appears before the quotation number (e.g., QT-1001).
                        </p>
                      </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-surface border border-outline-variant/30 rounded-[2rem] p-1">
                  <div className="relative h-full bg-surface-container-lowest rounded-[1.8rem] p-6 sm:p-8">
                    ...
                    <div className="mt-8 p-4 bg-surface-container-low rounded-xl border border-outline-variant/30 flex gap-3 items-start">

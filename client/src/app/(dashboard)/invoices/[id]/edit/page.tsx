    ...
    try {
      ...
      if (data.items && data.items.length > 0) {
        const mappedItems = data.items.map((item: any) => ({
          ...
          sku: item.productSnapshot?.skuNumber || item.productSnapshot?.sku || '',
          hsnCode: item.productSnapshot?.hsnNumber || item.productSnapshot?.hsnCode || '',
        }));
    ...
    return (
      <div className="flex-1 overflow-y-auto p-4 md:p-8 relative overflow-x-hidden selection:bg-primary/30">
        <div className="flex flex-col items-center justify-center py-32">
          ...
          <p className="mt-6 text-on-surface-variant font-medium tracking-wide">Loading invoice data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="relative z-10 max-w-7xl mx-auto flex flex-col gap-12 pb-16 w-full max-w-full min-w-0">
            ...
            <div className="flex flex-col gap-2">
              ...
              {invoiceNumber && (
                <span className="text-sm font-bold text-primary inline-flex items-center gap-1.5 bg-primary/5 px-2 py-1 rounded w-max border border-primary/10">
                  <span className="material-symbols-outlined text-[14px]">tag</span> {invoiceNumber}
                </span>
              )}
            </div>
          </div>
        ...
        {error && (
          <div ref={errorRef} className="mb-6 p-4 rounded-xl bg-red-50 border border-red-200 flex items-start gap-3 shadow-sm">
            ...
            <div className="text-sm text-red-700 whitespace-pre-line font-medium leading-relaxed">{error}</div>
          </div>
        )}

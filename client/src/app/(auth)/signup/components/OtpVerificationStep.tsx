  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="bg-primary/5 p-4 rounded-xl border border-primary/10 mb-6 text-sm text-on-surface-variant">
        We've sent an OTP to <strong>{formData.email}</strong> and <strong>+91 {formData.mobileNumber}</strong>. Please enter them below to verify your contact details.
        <br />
        <span className="text-xs text-amber-600 mt-1 block">Check the server console for mock OTPs.</span>
      </div>
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-semibold text-primary uppercase tracking-wider ml-1">Mobile OTP</label>
        <div className="flex items-center input-container rounded-full overflow-hidden input-glow px-4 py-1.5">
          <span className="material-symbols-outlined text-on-surface-variant mr-3 text-xl select-none">sms</span>
          <input
            type="text"
            maxLength={6}
            ...
            onChange={(e) => {
              const val = e.target.value.replace(/\D/g, '');
              clearError('mobileOtp');
              updateData({ mobileOtp: val });
            }}
            placeholder="6-digit code"
            className="w-full bg-transparent border-none text-on-surface placeholder-on-surface-variant/40 focus:ring-0 focus:outline-none py-2 text-sm tracking-[0.5em] font-mono"
          />
        </div>
        <AlertMessage message={errors.mobileOtp} />
      </div>
    </div>
  );
}
